...
#define PHDDNS_LINUX_CONFIGFILE "/mnt/jffs2/etc/phlinux.conf"
#define PHDDNS_WIN32_CONFIGFILE "d:\phlinux.ini"

#define PHDDNS_LINUX_LOGFILE "/mnt/jffs2/log/phddns.log"
#define PHDDNS_WIN32_LOGFILE "d:\\phclientlog.log"
...
#ifndef WIN32
	strcpy(parameter->szconfig,PHDDNS_LINUX_CONFIGFILE);
#else
	strcpy(parameter->szconfig,PHDDNS_WIN32_CONFIGFILE);
#endif
	for( i=1;i<argc;i++ ) {
...
{
#ifndef WIN32
	printf("Log to use(default %s):",PHDDNS_LINUX_LOGFILE);
#else
	printf("Log to use(default %s):",PHDDNS_WIN32_LOGFILE);
#endif

	memset(parameter->logfile,0,sizeof(parameter->logfile));
	fgets(parameter->logfile,100,stdin);
	if( strlen(phddns_trim(parameter->logfile)) == 0 ) {
#ifndef WIN32
		strcpy(parameter->logfile,PHDDNS_LINUX_LOGFILE);
#else
		strcpy(parameter->logfile,PHDDNS_WIN32_LOGFILE);
#endif
	}
	printf("%s\n",parameter->logfile);
	return;
...
void NewHost(PHGlobal *pglobal)
{
	while(1)
	{
		...
		if( strlen(phddns_trim(pglobal->szHost)) == 0 )
		{
...
void NewUserID(PHGlobal *pglobal)
{
	while(1)
	{
		...
		if( strlen(phddns_trim(pglobal->szUserID)) == 0 )
			continue;
		else
			break;
	}
...
void NewnicName(PHGlobal *pglobal,PH_parameter *parameter)
{
	...
	if(strlen(phddns_trim(parameter->nicName)) == 0)
			...
			if(strcmp(com,"y")==0||strcmp(com,"yes")==0||strcmp(com,"Y")==0)
			{
				...
				if(strcmp(com,"n")==0||strcmp(com,"no")==0||strcmp(com,"N")==0)
					...
					if(strcmp(com,"other")==0)
					{
						...
						fflush(stdin);
#ifndef WIN32
						printf("Enter configuration filename(%s):",PHDDNS_LINUX_CONFIGFILE);
#else
						printf("Enter configuration filename(%s):",PHDDNS_LINUX_CONFIGFILE);
#endif
						fgets(command,100,stdin);
						if( strlen(phddns_trim(command)) == 0 )
						{
...
int MyReadFile(char* filename,PHGlobal *pglobal,PH_parameter *parameter)
{
	...
	while( memset(buffer,0,sizeof(buffer)) && fgets(buffer,sizeof(buffer),fp) && circle < 5 ) {
		...
		strcpy(attribute,phddns_trim(pointer));
		if( strstr(attribute,"settings") != NULL )
			continue;

		pointer = strtok( NULL,"=");
		memset(tvalue,0,sizeof(tvalue));
		strcpy(tvalue,phddns_trim(pointer));
	...
	if(access(parameter->szconfig,0) < 0 ) {
		...
		while( memset(buffer,0,sizeof(buffer)) && fgets(buffer,sizeof(buffer),fp) ) {
			if( strstr(buffer,attribute) != NULL ) {
				...
				strcpy(attribute_copy,phddns_trim(pointer));
